...
snippet::io.micronaut.configuration.lettuce.docs.NamedConnectionCommands[tags="namedConnection", project-base="docs-examples/codecs"]
...
snippet::io.micronaut.configuration.lettuce.docs.NamedCodecFactory[tags="namedCodec,namedCodec2", project-base="docs-examples/codecs"]

== Redis Health Checks

When the `redis-lettuce` compatibility module or the focused `redis-lettuce-core` module is activated a api:configuration.lettuce.health.RedisHealthIndicator[] is activated resulting in the `/health` endpoint and https://docs.micronaut.io/latest/api/io/micronaut/health/CurrentHealthStatus.html[CurrentHealthStatus] interface resolving the health of the Redis connection or connections.
