...
dependency:micronaut-redis-lettuce-core[groupId="io.micronaut.redis"]

dependency:micronaut-redis-lettuce-cache[groupId="io.micronaut.redis"]

dependency:micronaut-redis-lettuce-session[groupId="io.micronaut.redis"]
...
snippet::io.micronaut.configuration.lettuce.docs.PooledRedisCommands[tags="pool", project-base="docs-examples/codecs"]
...
snippet::io.micronaut.configuration.lettuce.docs.RedisClientCommands[tags="commands", project-base="docs-examples/codecs"]
...
snippet::io.micronaut.configuration.lettuce.docs.ByteArrayCodecFactory[tags="codecFactory", project-base="docs-examples/codecs"]

A `RedisCodec` bean that is neither `@Primary` nor `@Named` does not replace the default `StringCodec`. Instead, `StatefulRedisConnection` and `StatefulRedisPubSubConnection` beans (or their cluster equivalents when `redis.uris` is configured) are registered for the codec's key and value types, and can be injected by those type arguments:

.Injecting a connection for a custom RedisCodec
snippet::io.micronaut.configuration.lettuce.docs.ByteArrayCodecTest[tags="typedConnection", project-base="docs-examples/codecs"]
